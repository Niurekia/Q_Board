...
public interface BoardRepository extends JpaRepository<Board,Long> {
    ...
    @Query(value = "SELECT * FROM Q_Board.board ORDER BY no DESC LIMIT :amount OFFSET :offset", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM Q_Board.board b WHERE b.title LIKE %:keyWord%  ORDER BY b.no DESC LIMIT :amount OFFSET :offset", nativeQuery = true)
    List<Board> findBoardTitleAmountStart(@Param("keyWord")String keyword, @Param("amount") int amount,@Param("offset") int offset);

    @Query(value = "SELECT * FROM Q_Board.board b WHERE b.username LIKE %:keyWord%  ORDER BY b.no DESC LIMIT :amount OFFSET :offset", nativeQuery = true)
    List<Board> findBoardUsernameAmountStart(@Param("keyWord")String keyword, @Param("amount") int amount,@Param("offset") int offset);

    @Query(value = "SELECT * FROM Q_Board.board b WHERE b.content LIKE %:keyWord%  ORDER BY b.no DESC LIMIT :amount OFFSET :offset", nativeQuery = true)
